...
class TripleImporterFromNxParser(name: String, namespace: String, ontologyNamespace: String, datasetFiles: List[String]) extends TripleImporter {

	@Override
	def importTriples(db: DatabaseConnection, adding: Boolean) = {
		for (dataset <- datasetFiles) {
			var nxp: NxParser = null
			importTriplesByLine(db, new NxParser(new FileInputStream(dataset)), adding)
		}
	}

	def importTriplesByLine(db: DatabaseConnection, nxp: NxParser, adding: Boolean): Unit = {
