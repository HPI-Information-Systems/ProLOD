...
class GraphLodImport(var db: DatabaseConnection, name : String, namespace: String, ontologyNamespace : String, files : List[String]) {
	...
	val graphLod = GraphLOD.loadDataset(name, files.asJava, namespace, ontologyNamespace, excludedNamespaces.asJava)
	db.getDatasets()
	db.insert
