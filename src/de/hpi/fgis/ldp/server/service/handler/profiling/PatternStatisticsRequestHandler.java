...
public class PatternStatisticsRequestHandler implements
    ActionHandler<PatternStatisticsRequest, DataTableResult> {
  ...
  @Override
  public DataTableResult execute(final PatternStatisticsRequest action,
      final ExecutionContext context) throws RPCException {
    final Cluster cluster = action.getCluster();
    logger.debug("managing pattern statistis request for cluster " + cluster.getId());
    final ArrayList<Predicate> predicates = action.getPredicates();
    final Datatype datatype = action.getDatatype();
    final NormalizedPattern normPattern = action.getNormalizedPattern();
    ...
    try {
      ...
      final IDataTable dataTypeStats =
          loader.getPattern(predicates, datatype, normPattern, from, to, debugProcess.get());
  ...
  @Override
  public Class<PatternStatisticsRequest> getActionType() {
    return PatternStatisticsRequest.class;
  }

  @Override
  public void rollback(PatternStatisticsRequest arg0, DataTableResult arg1, ExecutionContext arg2)
      throws RPCException {
